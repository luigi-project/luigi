...
export const LuigiContainerHelpers = {
  ...
  getMainMicrofrontends(luigi: Luigi): MicrofrontendEntry[] {
    ...
    for (const element of containerWrapper.childNodes as any) {
      ...
      if (element.iframeHandle?.iframe) {
        results.push({
          iframe: element.iframeHandle.iframe,
          id: element.luigiMfId,
          active: GenericHelpers.isElementVisible(element)
        });
      } else {
  ...
  getDrawerMicrofrontends(): MicrofrontendEntry {
    ...
    if (UIModule.drawerContainer.iframeHandle?.iframe) {
      return {
        iframe: UIModule.drawerContainer.iframeHandle.iframe,
        id: UIModule.drawerContainer.luigiMfId,
        active: GenericHelpers.isElementVisible(UIModule.drawerContainer)
      };
    } else {
